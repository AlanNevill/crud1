...
const nextYear = dateFns.getYear(
  new Date(dateFns.addYears(new Date(TODAY), 1))
);
...
function showBookings() {
  ...
  if (selectedYear == thisYear) {
    ...
    startDate = dateFns.format(
      dateFns.startOfYear(new Date(selectedYear, 1, 1, 23, 59, 0)),
      "YYYY-MM-DD"
    );
  }

  let yearEnd = dateFns.format(
    dateFns.endOfYear(new Date(selectedYear, 1, 1, 23, 59, 0)),
    "YYYY-MM-DD"
  );

  $.post("../include/newBooking_ajax.php", {
    ...
    if (funcReturn.success === true) {
      ...
      cottageWeekRows.forEach(cottageWeekRow => {
        ...
        let classForShortBreaks = cottageWeekRow.bShortBreaksAllowed
          ? "<button type='button' class='btn view_details'></button>"
          : "";
        ...
        let newRow = `
            ...
            <td style="width:20%" cottageNum='1'>${
              cottageWeekRow.CornflowerW
            }</td> 
            <td style="width:20%" cottageNum='2'>${
              cottageWeekRow.CowslipW
            }</td> 
            <td style="width:20%" cottageNum='3'>${
              cottageWeekRow.MeadowsweetW
            }</td> 
      ...
      if (funcReturn.cottageBookRows) {
        ...
        cottageBookRows.forEach(cottageBookRow => {
          setWeekBooked(
            cottageBookRow.DateSat,
            1,
            cottageBookRow.CornflowerNights
          );
          setWeekBooked(
            cottageBookRow.DateSat,
            2,
            cottageBookRow.CowslipNights
          );
          setWeekBooked(
            cottageBookRow.DateSat,
            3,
            cottageBookRow.MeadowsweetNights
          );
